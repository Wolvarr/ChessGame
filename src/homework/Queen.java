 ...
 * represents one single Queen figure
 */
public class Queen extends Figure {

    Queen(int posX, int posY, Colour c) {
        super(posX, posY, c);
    }
    ...
    public boolean validMoveCheck(int toX, int toY) {
        ...
        if (y - x != toY - toX && x + y != toX + toY && x != toX && y != toY)
            return false;
    ...
    public boolean moveTo(int toX, int toY) {
        if (!validMoveCheck(toX, toY)) {
            ...
            if (board.isEmpty(toX, toY)) {
                System.out.println(colour + " Queen moved to " + toX + " " + toY);
    ...
    @Override
    public void die() {
        System.out.println(colour + " Queen died");
        isAlive = false;
    }
